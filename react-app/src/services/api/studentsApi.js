// API service for students - replaces Firestore calls
...
export async function setStudent(uid, profile) {
  ...
  if (student) {
    // Update existing - ensure student.id is a valid number
    if (!student.id || isNaN(student.id)) {
      throw new Error('Invalid student ID. Please refresh the page and try again.')
    }
    
    // Prepare update data (exclude firebase_uid as it's not updatable via this endpoint)
    const updateData = {
      name: profile.name,
      email: profile.email,

...
import './Login.css'
import { registerWithEmail, signIn as firebaseSignIn, saveUserProfile, signOutUser } from '../../firebase'
...
function Login() {
  ...
  useEffect(() => {
    // Use prefixed key to avoid conflicts with static pages
  ...
  const handleLogin = async (e) => {
    ...
    if (!isValidEmail(loginData.email, !isProfessor)) {
    ...
    try {
      ...
      // Step 1: Authenticate (fastest operation)
      const credential = await firebaseSignIn(loginData.email.trim(), loginData.password)
      const { user } = credential

      // Step 2: Load profile in parallel with preparing user data
      const profilePromise = isProfessor 
        ? getProfessorByUid(user.uid) 
        : getStudentByUid(user.uid)

      // Prepare basic user data immediately (optimistic)
      ...
      // Step 3: Handle profile creation/update (optimize for speed)
      if (!profile) {
        // For new users, generate avatar quickly and save
        ...
        // Save profile (don't await - let it happen in background)
        ...
        // For existing profiles without photo, generate and save in background (non-blocking)
        const defaultPhotoURL = getDefaultAvatar(
          profile.name || fallbackName,
          user.uid
        )
        profile.photoURL = defaultPhotoURL
        // Update in background - don't block navigation
      ...
      // Step 4: Prepare user data and navigate immediately
      // Use profile name from MySQL, fallback to session or email
      const displayName = profile?.name || fallbackName
      const userData = {
        uid: user.uid,
        type: CONFIG.USER_TYPES[isProfessor ? 'PROFESSOR' : 'STUDENT'],
        email: user.email,
        name: displayName, // Use the name from MySQL profile
        ...(isProfessor
      ...
      // Step 5: Persist session and navigate immediately
      persistSessionUser(userData)
      navigate(isProfessor ? '/prof' : '/student')
    } catch (error) {
  ...
  const handleSignUp = async (e) => {
    ...
    if (!isValidPassword(signUpData.password)) {
      setAuthError('Password must be at least 8 characters with uppercase, lowercase, numbers, and symbols')
      return
    }
    ...
    try {
      ...
      // Generate unique default avatar for new user
      ...
      // Create profile in MySQL database
      try {
        if (isProfessor) {
        ...
        // If MySQL API fails, delete the Firebase user to prevent orphaned accounts
        try {
          await signOutUser()
          await user.delete()
        } catch (cleanupError) {
      ...
      // Save to Firebase (for backward compatibility - non-critical)
      try {
        await saveUserProfile(user.uid, profilePayload)
      } catch (firestoreError) {
        // Non-critical - profile is already in MySQL
        console.warn('Failed to save to Firestore (non-critical):', firestoreError)
      }

      // Sign out the user after account creation to require login
      try {
        await signOutUser()
      } catch (signOutError) {
        console.warn('Sign out error (non-critical):', signOutError)
        // Continue even if sign-out fails - user will need to log in anyway
      }
      ...
      // Clear any existing login input so Sign In form is empty for both users
      setProfessorLogin({ email: '', password: '' })
      setStudentLogin({ email: '', password: '' })

      // Reset form state - switch to login form first
      setIsSubmitting(false)
      setIsSignUp(false)
  ...
  return (
    <div className="application-background min-h-screen relative">
      ...
      {/* Application Header (fixed at top, login card has padding to avoid overlap) */}
        ...
        <div className="header-content-container max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="header-navigation-content flex justify-between items-center h-16">
            <div className="university-branding-section flex items-center space-x-3">
              ...
              <div>
                <h1 className="application-title text-xl font-bold university-primary-color leading-tight navigation-text">Student iTrack</h1>
      ...
      <div className="authentication-container relative z-10">
        <div className="authentication-card max-w-md w-full space-y-8 p-8">
          ...
          <div className="user-type-selection mb-6">
            <label className="selection-label block text-sm font-medium mb-3 text-gray-800">Login As</label>
            ...
            <div className="auth-form-section">
              <form onSubmit={handleLogin} className="authentication-form space-y-6 w-full">
                {userType === 'professor' ? (
                  <>
                    <div className="form-field-group">
                      <label className="form-label block text-sm font-medium text-gray-800">Email</label>
                      <input
                        type="email"
                        autoComplete="off"
                    ...
                    <div className="form-field-group mt-4">
                      <label className="form-label block text-sm font-medium text-gray-800">Password</label>
                  ...
                  <>
                    <div className="form-field-group">
                      <label className="form-label block text-sm font-medium text-gray-800">Student Email</label>
                      <input
                        type="email"
                        autoComplete="off"
                ...
                <div className="form-options-container">
                  ...
                  <a href="#" className="password-recovery-link text-sm font-medium">Forgot Password?</a>
                </div>
            ...
            <div className="auth-form-section">
              <form onSubmit={handleSignUp} className="registration-form space-y-6">
                {userType === 'professor' ? (
                  <>
                    <div className="form-field-group">
                      <label className="form-label block text-sm font-medium">Full Name</label>
                      <input
                        type="text"
                        autoComplete="off"
                    ...
                    <div className="form-field-group mt-4">
                      <label className="form-label block text-sm font-medium">Department</label>
                      <select
                    ...
                    <div className="form-field-group mt-4">
                      <label className="form-label block text-sm font-medium">Password</label>
                      <div className="password-input-container">
                        <input
                    ...
                    <div className="form-field-group mt-4">
                      <label className="form-label block text-sm font-medium">Confirm Password</label>
                      <div className="password-input-container">
                        <input
                  ...
                  <>
                    ...
                    <div className="form-field-group mt-4">
                      <label className="form-label block text-sm font-medium">Student ID</label>
                      <input
                        type="text"
                        autoComplete="off"
          ...
          <div className="terms-modal-container">
            ...
            <div className="terms-modal-content">
              <div className="terms-content">
                ...
                {/* Terms content - truncated for brevity, include full content from original */}
                ...
                {/* UM Website Terms and Conditions */}

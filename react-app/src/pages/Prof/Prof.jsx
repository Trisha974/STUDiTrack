...
import { getStudentByEmail, getStudentByNumericalId, setStudent, addStudent } from '../../services/students'
import {
  syncStudentGrade,
...
import { getCourseByCode, createCourse } from '../../services/courses'
import { createEnrollment, getEnrollmentByStudentAndCourse, subscribeToCourseEnrollments } from '../../services/enrollments'
...
function Prof() {
    ...
    try {
      ...
      if (saved) {
        ...
        if (missingEnrollmentStudents.length > 0) {
          console.error('❌ CRITICAL: Found enrollments for students that don\'t exist in students array:', {
          ...
          console.error('⚠️ WARNING: This data inconsistency will cause students to disappear on refresh!')
          console.error('💡 The enrollments will be kept, but students won\'t display until students are added')
        } else {
  ...
  const executeSave = useCallback(async (payload, uidToUse) => {
    try {
      ...
      if (missingStudents.length > 0) {
        console.error('❌ CRITICAL: Attempting to save enrollments for students that don\'t exist:', {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToProfessorDashboard(profUid, (updatedData) => {
      ...
      if (updatedData.enrolls && Object.keys(enrolls).length > 0) {
        ...
        for (const subjectCode in enrolls) {
          ...
          if (incomingEnrolls.length < currentEnrolls.length) {
            console.log('⏸️ Real-time update skipped: Incoming data appears stale (fewer enrollments)', {
              subject: subjectCode,
          ...
          if (missingStudents.length > 0 && currentEnrolls.length > 0) {
            console.log('⏸️ Real-time update skipped: Incoming data missing students', {
              subject: subjectCode,
              incoming: incomingEnrolls.length,
              current: currentEnrolls.length,
      ...
      if (updatedData.enrolls) {
        ...
        console.log('🔄 Real-time update: Enrollments normalized and updated', {
  ...
  const handleSaveAllGrades = async () => {
    ...
    try {
      ...
      if (savedCount > 0 && failedCount === 0) {
        ...
        const errorMsg = `Grades saved for ${savedCount} student(s), but failed for ${failedCount} student(s): ${failedStudents.join(', ')}. Check console for details.`
        console.warn('⚠️ Partial save:', errorMsg)
        addCustomAlert('warning', 'Partial Save', errorMsg, false)
      } else if (savedCount === 0) {
        const errorMsg = 'No grades were saved. Please check that students exist in the system and try again.'
        console.error('❌ No grades saved:', errorDetails)
        addCustomAlert('error', 'No Grades Saved', errorMsg, false)
      ...
      addCustomAlert('error', 'Save Failed', `Unable to save grades: ${error.message || 'An unexpected error occurred'}. Please try again.`, false)
    }
  }
      ...
      if (!studentUid) {
        ...
        for (const subjectCode of newStudent.subjects) {
          ...
          if (!existingEnrollment) {
            ...
            console.log(`✅ Created enrollment: Student ${studentToUse.id} → Course ${subjectCode}`)
          }
        }
    ...
    if (student && subject) {
      try {
        ...
        if (!studentData) {
          ...
          studentData = { id: studentMySQLId, student_id: student.id, name: student.name, email: student.email, firebase_uid: firebaseUid }
          console.log(`✅ Created student in MySQL: ${student.name} (MySQL ID: ${studentMySQLId}, Firebase UID: ${firebaseUid})`)
        } else {
        ...
        console.error('❌ Failed to create enrollment in MySQL:', error)
        addCustomAlert('error', 'Enrollment Error', `Failed to create enrollment: ${error.message || 'An unexpected error occurred'}. The student was added to the subject locally, but may not appear on their dashboard until this is fixed.`, false)
      }
    }
  ...
  const handleArchiveStudent = async (studentId, subjectCode) => {
    ...
    // After successful add, clear selection/inputs so another student can be added smoothly
    setStudentToAdd('')
    setNewStudentQuick({ id: '', name: '', email: '' })
    setIsAddingStudentToSubject(false)
      }
    ...
    try {
      ...
      if (studentStillInEnrolls) {
        console.error(`❌ ERROR: Student ${normalizedId} still in enrolls[${subjectCode}] before save!`)
        // Force remove again
        updatedEnrolls[subjectCode] = updatedEnrolls[subjectCode].filter(id => normalizeStudentId(id) !== normalizedId)
      }
      ...
      if (isInEnrolls) {
        console.error(`❌ CRITICAL ERROR: Student ${normalizedId} still in enrolls before save!`)
        updatedEnrolls[subjectCode] = updatedEnrolls[subjectCode].filter(id => normalizeStudentId(id) !== normalizedId)
      }
  ...
  const handleImportCSV = async () => {
    ...
    try {
      ...
      setNormalizedEnrolls(prevEnrolls => {
        ...
        console.log('🔄 Post-save state update:', {
          subject: studentSubjectFilter,
      ...
      isImportingRef.current = false
      console.log('✅ Import flag cleared - students should now be visible in UI')
    }
                      ...
                      <div className="flex items-start justify-between w-full">
                        <div className="flex items-center space-x-4">
                          ...
                          <div className="flex flex-col">
                            ...
                            <div className="text-xs text-slate-400">{subject.code} • {subject.credits} Credits</div>
                  ...
                  .map(subject => {
                    ...
                    const currentStudentObjects = normalizedEnrollments
                      .map(id => {
                        ...
                        if (!student) {
                          console.warn(`⚠️ Student ID ${id} is enrolled in ${subject.code} but not found in students array`, {
                            enrollmentId: id,
                ...
                <button
                  onClick={async () => {
                    try {
                      ...
                      if (!saveResult) {
                        console.error('❌ CRITICAL: Restore save operation returned false - data may not have been saved!')
          ...
          <div className={`glass rounded-2xl p-8 w-full max-w-md shadow-2xl ${
            ...
            <div className="flex justify-end space-x-4 mt-8">
              ...
              <button
                ...
                disabled={isAddingStudentToSubject}
                className={`btn text-white px-6 py-3 rounded-xl font-semibold shadowLg hover:shadow-xl transition-all duration-300 ${
                  isAddingStudentToSubject ? 'opacity-60 cursor-not-allowed' : ''
                }`}
              >
                {isAddingStudentToSubject ? 'Adding…' : 'Add Student'}
              </button>
            </div>
          </div>
        </div>

# Create Database and Configure Backend
...
- ✅ You're in MySQL monitor (`mysql>` prompt)

---

## Step 1: Create Database

**While in MySQL monitor (`mysql>`), run these commands:**
...
**Expected output:**
- You should see `student_itrack` in the list of databases

**If you see `student_itrack`:**
...
- `DB_PASSWORD=` (leave empty if you didn't use a password, or set it if you have one)
...
```bash
# 1. In MySQL monitor, create database:
CREATE DATABASE IF NOT EXISTS student_itrack;
SHOW DATABASES;
EXIT;
...
**Error:** `ERROR 1007 (HY000): Can't create database 'student_itrack'; database exists`

**Solution:**
- This is OK! Database already exists
- Continue to Step 2
...
**Next:** Create the database, then update `.env` and restart the backend!
